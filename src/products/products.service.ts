...
export class ProductsService {
  ...
  async findAll(query: any): Promise<{ products: any[], totalProducts, limit }> {
    let filteredTocalProducts: number;
    let limit: number;
    if (!query.limit) {
      limit = 4;
    } else {
  ...
  //     // để sao chép tất cả các thuộc tính của đối tượng updateProductDto (các trường cần được cập nhật) vào đối tượng product đã tìm thấy.
  //     // Bằng cách này, chỉ những thuộc tính có trong updateProductDto mới được cập nhật, còn các thuộc tính không có trong updateProductDto sẽ giữ nguyên giá trị cũ.
  ...
  async update(
    ...
    // Sao chép tất cả các thuộc tính từ updateProductDto vào product
    Object.assign(product, updateProductDto);
    product.addedBy = currentUser;

    // Nếu có categoryId trong updateProductDto, tìm và cập nhật category
    if (updateProductDto.categoryId) {
      const category = await this.categoryService.findOne(+updateProductDto.categoryId);
      product.category = category;
    }

    // Lưu lại các thay đổi trong repository
    return await this.productRepository.save(product);
  }
